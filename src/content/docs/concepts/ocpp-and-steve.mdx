...
[SteVe](/glossary#steve) — an open-source OCPP central system the
...
In OCPP, the [ChargeBox](/glossary#chargebox) is the client. It opens
...
1. **SteVe REST API → Polaris (pull).** Polaris polls SteVe for
   ...
   reservations. This is the read path. See `web/src/lib/steve-client.ts`.
...
3. **SteVe → Polaris (push).** Our fork adds two outbound webhooks
   ...
   - The [pre-authorize](/glossary#pre-authorize) hook fires on
     `Authorize` (and on `StartTransaction` when an idTag is present)
...
**Why webhooks for meter values, REST polling for everything else?**
