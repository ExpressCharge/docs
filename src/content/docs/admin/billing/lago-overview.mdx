...
is a thin projection of state held in [Lago](/glossary#lago) — an
...
A [Lago customer](/glossary#lago-customer) is the billing identity for
one Polaris account. We address customers by their `external_id`, which
is always the Polaris user's [Public ID](/glossary#public-id) — never
...
*charges*, each pointing at a [Lago metric](/glossary#lago-metric) with
...
A [Lago subscription](/glossary#lago-subscription) attaches one customer
...
- [Charging profile metadata mirror](/concepts/charging-profile-mirror/)