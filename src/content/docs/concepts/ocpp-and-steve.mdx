...
[<Glossary term="<Glossary term="SteVe">SteVe</Glossary>">SteVe</Glossary>](/glossary#steve) — an open-source <Glossary term="<Glossary term="OCPP">OCPP</Glossary>">OCPP</Glossary> central system the
...
In OCPP, the [<Glossary term="<Glossary term="ChargeBox">ChargeBox</Glossary>">ChargeBox</Glossary>](/glossary#chargebox) is the client. It opens
...
1. **SteVe REST API → Polaris (pull).** Polaris polls SteVe for
   ...
   <Glossary term="<Glossary term="Reservation">Reservation</Glossary>">reservations</Glossary>. This is the read path. See `web/src/lib/steve-client.ts`.
...
3. **SteVe → Polaris (push).** Our fork adds two outbound webhooks
   ...
   - The [pre-authorize](/glossary#pre-authorize) hook fires on
     `Authorize` (and on `StartTransaction` when an <Glossary term="<Glossary term="idTag">idTag</Glossary>">idTag</Glossary> is present)
...
**Why webhooks for <Glossary term="<Glossary term="Meter value">Meter value</Glossary>">meter values</Glossary>, REST polling for everything else?**
