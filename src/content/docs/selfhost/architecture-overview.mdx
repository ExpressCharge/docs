...
A Polaris Express deployment is two stacks running side by side: the **Polaris stack** (the web app, sync worker, and its Postgres) and the **<Glossary term="<Glossary term="<Glossary term="SteVe">SteVe</Glossary>">SteVe</Glossary>">SteVe</Glossary> stack** (the <Glossary term="<Glossary term="<Glossary term="OCPP">OCPP</Glossary>">OCPP</Glossary>">OCPP</Glossary> server and its MariaDB). They are deliberately separate so that the OCPP server can be restarted, upgraded, or replaced without touching application state.
...
- **`postgres`** — Postgres 17. Source of truth for users, ChargeBoxes, <Glossary term="<Glossary term="<Glossary term="EV card">EV card</Glossary>">EV card</Glossary>">EV cards</Glossary>, idTags, tariffs, <Glossary term="<Glossary term="<Glossary term="Sync run">Sync run</Glossary>">Sync run</Glossary>">sync runs</Glossary>, and the audit log. The named volume `postgres_data` is the only piece of state Polaris itself owns.
...
- **`app`** — the Deno web app. Serves the dashboard, the public booking surfaces, the API, and the <Glossary term="<Glossary term="<Glossary term="Kiosk mode">Kiosk mode</Glossary>">Kiosk mode</Glossary>">kiosk</Glossary> views. Mounts the Docker socket so it can introspect the stack from within. Exposed on port 8000.
- **`sync`** — a long-running Deno process (`sync-worker.ts`) that reconciles state between Polaris and SteVe. ChargeBoxes, idTags, and <Glossary term="<Glossary term="<Glossary term="Reservation">Reservation</Glossary>">Reservation</Glossary>">reservations</Glossary> created in Polaris become OCPP entities in SteVe via this worker; transactions and <Glossary term="<Glossary term="<Glossary term="Meter value">Meter value</Glossary>">Meter value</Glossary>">meter values</Glossary> flow back.
...
- **`mariadb`** — MariaDB 10.4. Holds SteVe's own data: <Glossary term="<Glossary term="<Glossary term="ChargeBox">ChargeBox</Glossary>">ChargeBox</Glossary>">charge box</Glossary> registrations, OCPP idTags, transactions, meter values. Bind-mounted to `/data/ocpp/mariadb` on the host.
