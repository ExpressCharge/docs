...
|---|---|---|---|
| Postgres 17 | `postgres` (from `docker-compose.yml`) | `postgres_data` | Users, [ChargeBoxes](/glossary#chargebox), [tariffs](/glossary#tariff), [sync runs](/glossary#sync-run), [audit log](/glossary#audit-log), [Lago](/glossary#lago) mappings, [EV cards](/glossary#ev-card), [reservations](/glossary#reservation) |
| MariaDB 10.4 | `mariadb` (from `steve/docker-compose.yml`) | `/data/ocpp/mariadb` (host bind mount) | [SteVe](/glossary#steve) charge boxes, OCPP transactions, [idTags](/glossary#idtag), [meter values](/glossary#meter-value), connector status |
