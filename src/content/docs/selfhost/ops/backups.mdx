...
Polaris Express keeps state in two databases: a Postgres instance (the web app, billing sync, audit log, <Glossary term="<Glossary term="Sync run">Sync run</Glossary>">sync runs</Glossary>) and a MariaDB instance (<Glossary term="<Glossary term="SteVe">SteVe</Glossary>">SteVe</Glossary> — <Glossary term="<Glossary term="OCPP">OCPP</Glossary>">OCPP</Glossary> <Glossary term="<Glossary term="ChargeBox">ChargeBox</Glossary>">charge box</Glossary> registry, transactions, idTags). Both must be backed up. Losing either one breaks the platform; losing them together is unrecoverable.
...
- **Before destructive ops** — migrations that drop columns, manual data fixes, large <Glossary term="<Glossary term="idTag">idTag</Glossary>">idTag</Glossary> re-imports.

## What gets backed up

| Database | Container | Volume | Contents |
|---|---|---|---|
| Postgres 17 | `postgres` (from `docker-compose.yml`) | `postgres_data` | Users, [ChargeBoxes](/glossary#chargebox), [tariffs](/glossary#tariff), [sync runs](/glossary#sync-run), [audit log](/glossary#audit-log), [<Glossary term="<Glossary term="Lago">Lago</Glossary>">Lago</Glossary>](/glossary#lago) mappings, [<Glossary term="<Glossary term="EV card">EV card</Glossary>">EV cards</Glossary>](/glossary#ev-card), [<Glossary term="<Glossary term="Reservation">Reservation</Glossary>">reservations</Glossary>](/glossary#reservation) |
| MariaDB 10.4 | `mariadb` (from `steve/docker-compose.yml`) | `/data/ocpp/mariadb` (host bind mount) | [SteVe](/glossary#steve) charge boxes, OCPP transactions, [idTags](/glossary#idtag), [<Glossary term="<Glossary term="Meter value">Meter value</Glossary>">meter values</Glossary>](/glossary#meter-value), connector status |
