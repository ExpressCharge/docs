...
- **Unmanaged** — chargers whose `managementMode` is `unmanaged` (i.e. not connected over [OCPP](/glossary#ocpp) via [<Glossary term="<Glossary term="<Glossary term="SteVe">SteVe</Glossary>">SteVe</Glossary>">SteVe</Glossary>](/glossary#steve)).
...
Each charger card shows the [<Glossary term="<Glossary term="<Glossary term="ChargeBox">ChargeBox</Glossary>">ChargeBox</Glossary>">ChargeBox</Glossary>](/glossary#chargebox) ID, friendly name, last status, form factor, and capability pills. Each scanner card shows the device label, platform, model, app version, and owner.
...
**<Glossary term="<Glossary term="<Glossary term="Capability">Capability</Glossary>">Capability</Glossary>">Capability</Glossary> pill missing on a charger.** The `'charger'` capability is force-added in the loader even if the database row is misconfigured, so this should never happen. If it does, file a bug with the `chargeBoxId`.
