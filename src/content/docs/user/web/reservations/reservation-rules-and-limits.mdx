...
A [Reservation](/glossary/#reservation) holds a specific [Connector](/glossary/#connector) on a specific [ChargeBox](/glossary/#chargebox) for a window of time, tied to one of your [EV cards](/glossary/#ev-card). Understanding what the system enforces — and what it doesn't — helps you read 4xx responses and the suggestion chips that appear when your first choice is taken.

## The model

A reservation is the tuple `(chargeBox, connector, card, startAt, endAt)`. Every reservation is owned by exactly one card; the card determines who can see it and which [Lago subscription](/glossary/#lago-subscription) the eventual session bills against.
...
2. **Capability.** Creating a reservation requires the `reserve` [Capability](/glossary/#capability). Drivers whose accounts are inactive get `403 Account inactive` with the capability name attached, regardless of whether the requested window is free.
...
There is no minimum or maximum duration enforced at this layer beyond `endAt > startAt`. Practical limits come from the [Tariff](/glossary/#tariff) and from how long the [ChargeBox](/glossary/#chargebox) will actually hold an [OCPP](/glossary/#ocpp) reservation — both of which can shorten what you booked, but neither will prevent the booking itself.
...
**Cards are the unit of ownership.** Routing scope through [idTag](/glossary/#idtag) primary keys (rather than user IDs) means the same checks work for shared cards, multi-user fleets, and future cases where one human holds cards across multiple tenants. The customer API never asks "is this your reservation?" — it asks "is this your card?" and the reservation follows.
...
**Impersonation is read-only.** An operator using [Impersonation](/glossary/#impersonation) can browse a customer's reservations to help them troubleshoot, but cannot create one on their behalf. Mutations under an impersonated session return `403`. Operators who need to book on behalf of a driver use admin tools, which carry their own [audit log](/glossary/#audit-log) trail.
