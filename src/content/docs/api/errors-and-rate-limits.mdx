...
| `422` | Request was well-formed but semantically invalid (e.g. referenced a <Glossary term="Lago">Lago</Glossary> customer that is not active). |
| `429` | Rate limit exceeded. See below. |
| `500` | Unhandled server error. Safe to retry with backoff. |
| `502` / `503` / `504` | Upstream dependency (<Glossary term="SteVe">SteVe</Glossary>, Lago, mail provider) failed or timed out. Retry with backoff. |
...
| --- | --- | --- |
| Auth (`/api/auth/*`, <Glossary term="Magic link">magic-link</Glossary> send, password reset) | 10 requests | 1 minute |
...
| <Glossary term="OCPP">OCPP</Glossary> webhook receiver | unbounded (gated by shared secret) | — |
