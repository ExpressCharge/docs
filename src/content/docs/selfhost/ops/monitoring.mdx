...
- iOS app (device)
  ...
  - → <Glossary term="<Glossary term="SSE">SSE</Glossary>">SSE</Glossary> `/api/admin/devices/{id}/logs-stream`
</FileTree>
...
records on each [<Glossary term="<Glossary term="Sync run">Sync run</Glossary>">Sync run</Glossary>](/glossary#sync-run). The server inserts
...
- `device_logs.observed_ts` for the latest row per device is within
  the device's expected [<Glossary term="<Glossary term="Adaptive Cadence">Adaptive Cadence</Glossary>">Adaptive Cadence</Glossary>](/glossary#adaptive-cadence)
