...
[<Glossary term="SteVe">SteVe</Glossary>](/glossary#steve) — an open-source <Glossary term="OCPP">OCPP</Glossary> central system the
...
In OCPP, the [<Glossary term="ChargeBox">ChargeBox</Glossary>](/glossary#chargebox) is the client. It opens
...
1. **SteVe REST API → Polaris (pull).** Polaris polls SteVe for
   ...
   <Glossary term="Reservation">reservations</Glossary>. This is the read path. See `web/src/lib/steve-client.ts`.
...
3. **SteVe → Polaris (push).** Our fork adds two outbound webhooks
   ...
   - The [pre-authorize](/glossary#pre-authorize) hook fires on
     `Authorize` (and on `StartTransaction` when an <Glossary term="idTag">idTag</Glossary> is present)
...
**Why webhooks for <Glossary term="Meter value">meter values</Glossary>, REST polling for everything else?**
