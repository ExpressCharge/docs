...
You have a running stack: <abbr title="Polaris Express web app">web</abbr>, SteVe, the database, and the email Worker. Now you need to register at least one ChargeBox in SteVe and point a real (or simulated) charge point at it. Until you do, the Polaris Express dashboard has nothing to display and no transactions to authorize.
...
<Aside type="caution">
  These credentials are SteVe's, not Polaris Express's. Polaris Express has its own admin login via BetterAuth (set up in an earlier phase). Do not reuse the same password.
</Aside>
...
Polaris Express reads <EnvVar name="STEVE_DATABASE_URL" /> to query the SteVe schema directly for ChargeBox, Connector, and transaction data. If you change SteVe's database credentials, also update the web `.env` and restart the web container.
...
<Aside type="danger">
  Do **not** delete a ChargeBox from SteVe to "reset" it. Deleting cascades to transactions, meter values, and reservations referencing it, which will leave the Polaris Express dashboard with broken references. To re-register a charge point, edit the existing row or contact your operator.
</Aside>
